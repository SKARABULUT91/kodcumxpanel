...
export default function CampaignsPage() {
  const { campaigns, addCampaign, updateCampaign, removeCampaign, addLog, setRunning, updateStats, accounts, settings } = useStore();
  ...
  const handleRun = async (campaign: Campaign) => {
    if (!activeAccount || activeAccount.status !== 'running') {
      addLog(`❌ Kampanya başlatılamadı — aktif hesap yok.`, 'error');
      return;
    }
    ...
    for (let t = 0; t < totalTweets; t++) {
      for (const step of campaign.steps) {
        completed++;
        if (step.action === 'like') updateStats({ likes: useStore.getState().stats.likes + 1 });
        else if (step.action === 'retweet') updateStats({ rts: useStore.getState().stats.rts + 1 });
        else if (step.action === 'follow') updateStats({ follows: useStore.getState().stats.follows + 1 });

        updateCampaign(campaign.id, { completedActions: completed });
        addLog(`✅ Kampanya adımı: ${actionLabels[step.action]} (${completed})`, 'success');

        await new Promise(r => setTimeout(r, step.delay * 1000));

        if (settings.antiShadowbanEnabled && completed % settings.actionsBeforeBreak === 0) {
          addLog(`⏸️ Anti-shadowban dinlenme...`, 'info');
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <p className="text-sm text-muted-foreground">Çok adımlı kampanyalar: Beğen → RT → Yorum → Takip.</p>
        <Button onClick={() => setShowCreate(!showCreate)} size="sm"><Plus className="w-4 h-4 mr-1.5" /> Yeni Kampanya</Button>
      </div>

      {showCreate && (
        <div className="bg-card border border-border rounded-lg p-5 animate-fade-in">
          <div className="text-xs font-semibold uppercase tracking-wider text-muted-foreground mb-4">🎯 Yeni Kampanya</div>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-4">
            <div><Label>Kampanya Adı</Label><Input placeholder="Ör: Yeni Ürün Lansmanı" value={name} onChange={(e) => setName(e.target.value)} /></div>
            <div><Label>Hedef (Kullanıcı / Hashtag)</Label><Input placeholder="@user veya #hashtag" value={target} onChange={(e) => setTarget(e.target.value)} /></div>
          </div>
          <div className="mb-4">
            <Label>Hesap Seçimi</Label>
            <select value={selectedAccountId} onChange={(e) => setSelectedAccountId(e.target.value)} className="flex h-10 w-full rounded-md border border-input bg-background px-3 py-2 text-sm">
              <option value="">Varsayılan (Aktif Hesap)</option>
              {accounts.map((acc) => (<option key={acc.id} value={acc.id}>@{acc.username} {acc.isActive ? '(Aktif)' : ''}</option>))}
            </select>
          </div>
                ...
                {step.action === 'comment' && step.enabled && (
                  <div className="mt-2 pl-7 grid grid-cols-1 sm:grid-cols-2 gap-2">
                    <div>
                      ...
                      <select value={step.commentStyle || 'samimi'} onChange={(e) => updateStep(idx, { commentStyle: e.target.value as CampaignStep['commentStyle'] })} className="flex h-8 w-full rounded-md border border-input bg-background px-2 py-1 text-xs">
                        {Object.entries(styleLabels).map(([k, v]) => (<option key={k} value={k}>{v}</option>))}
                      </select>
                    </div>
                    <div>
                      <Label className="text-xs">Sabit Metin (Boşsa AI üretir)</Label>
                      <Input className="h-8 text-xs" placeholder="Harika!" value={step.commentText || ''} onChange={(e) => updateStep(idx, { commentText: e.target.value })} />
                    </div>
                  </div>
                )}
              </div>
          ...
          <div className="flex gap-2">
            <Button onClick={handleCreate} disabled={!name.trim() || !target.trim() || !steps.some((s) => s.enabled)}><Zap className="w-4 h-4 mr-1.5" /> Oluştur</Button>
          ...
          {campaigns.map((c) => (
            <div key={c.id} className="bg-card border border-border rounded-lg p-5">
              <div className="flex items-center justify-between mb-3">
                ...
                <span className={cn('text-[10px] font-medium px-2 py-0.5 rounded-full', statusConfig[c.status].class)}>{statusConfig[c.status].label}</span>
              </div>
              <div className="flex gap-1.5 mb-3">
                {c.steps.map((s) => (<span key={s.action} className="text-[10px] bg-secondary text-muted-foreground px-2 py-0.5 rounded">{actionLabels[s.action]}</span>))}
              </div>
              <div className="mb-3">
              ...
              <div className="flex gap-2">
                {(c.status === 'draft' || c.status === 'paused') && (<Button size="sm" className="text-xs" onClick={() => handleRun(c)}><Play className="w-3 h-3 mr-1" /> Başlat</Button>)}
                {c.status === 'running' && (<Button size="sm" variant="secondary" className="text-xs" onClick={() => { updateCampaign(c.id, { status: 'paused' }); setRunning(false); }}><Pause className="w-3 h-3 mr-1" /> Duraklat</Button>)}
                <Button size="sm" variant="outline" className="text-xs border-destructive/20 text-destructive" onClick={() => removeCampaign(c.id)}><Trash2 className="w-3 h-3 mr-1" /> Sil</Button>
              </div>
            </div>
          ))}
        </div>
